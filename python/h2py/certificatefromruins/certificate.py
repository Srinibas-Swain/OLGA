# run it as python3 certificatefromruins.py <verbosity-level>
...
from h2py.certificatefromruins.blocks import *
...
class Certificate:
	...
	@classmethod
	def generate(cls, graph, verbosity=0):
		cert = Certificate()
		cert.verbosity = verbosity
		p = cert.initialize(graph)
		cert.canon2(p)
	...
	def canon2(self, p):
		...
		if q.is_discrete():
			self.handle_discrete_q(q, pi1, result)
		else:
			self.handle_non_discrete_q(q, l, result)
		self.str_verb_indent = self.str_verb_indent[:-3]

	def handle_discrete_q(self, q, pi1, result):
		(self.verbosity>=4) and print(self.str_verb_indent + 'handle_discrete_q Q', q)
	...
	def handle_non_discrete_q(self, Q, l, result):
		if result == 'worse':
			(self.verbosity>=2) and print(self.str_verb_indent + 'worse:', Q)
			return
		...
		(self.verbosity>=4) and print(self.str_verb_indent + 'handle_non_discrete_q R', R, ' C', C)
...
if __name__ == '__main__':
	...
	cert_val = Certificate.generate(graph, verbosity=int(sys.argv[1]))
	graph.generate_graph_id()
	
	print('#edges:', graph.num_edges(), ' graph-id:', graph.graph_id, ' certificatefromruins:', cert_val)
